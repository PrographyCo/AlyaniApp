...
if ($_POST) {
	...
	if ($sql->execute()) {
		...
		if ($_FILES['emp_uphoto']['tmp_name']) {
			...
			if (copy($_FILES['emp_uphoto']['tmp_name'], 'media/emps_photos/'.$id.'.'.$ext)) {
                ...
                <div class="box-body">
									<form role="form" method="post" enctype="multipart/form-data">
										...
										<div class="form-group">
											...
											<input type="text" class="form-control" name="emp_name" required="required" value="<? echo $_POST['emp_name'] ?: $row['emp_name']?>" />
										</div>

										<div class="form-group">
											<label><?=LBL_JobTitle;?></label>
											<input type="text" class="form-control" name="emp_jobtitle" required="required" value="<? echo $_POST['emp_jobtitle'] ?: $row['emp_jobtitle']?>" />
										</div>

										<div class="form-group">
											<label><?=LBL_JobID;?></label>
											<input type="text" class="form-control" name="emp_jobid" required="required" value="<? echo $_POST['emp_jobid'] ?: $row['emp_jobid']?>" />
										</div>

										<div class="form-group">
											<label><?=LBL_Photo;?></label><br />
											<? if ($row['emp_photo']) echo '<img src="media/emps_photos/'.$row['emp_photo'].'" width="150"/>'; ?>
										...
										<input type="hidden" name="emp_dateadded" id="emp_dateadded" value="<?=$row['emp_dateadded'];?>" />
