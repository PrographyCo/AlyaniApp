<?php include 'layout/header.php';
    ...
    $table = 'haj_album';
    $table_id = 'hp_id';
    $newedit_page = 'e_hp.php';
    
    if (is_numeric($_GET['del'])) {
        
        $id = $_GET['del'];
        $photo = $db->query("SELECT hp_photo FROM $table WHERE $table_id = $id")->fetchColumn();
        if ($photo && $photo != 'default_photo.png' && is_file('media/hajalbum/' . $photo)) @unlink('media/hajalbum/' . $photo);
                        ...
                        <table class="datatable-table4 table table-bordered table-striped">
                            ...
                            <?
                                ...
                                while ($row = $sql->fetch()) {
                                    ...
                                    echo '<tr>';
                                    echo '<td>';
                                    echo '<img src="media/hajalbum/' . $row['hp_photo'] . '" style="width:100px;" />';
                                    echo '</td>';
                                    echo '<td>';
