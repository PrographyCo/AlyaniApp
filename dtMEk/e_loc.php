...
if ($_POST) {
	...
	$sql->bindValue("loc_active", ($_POST['loc_active'] ? 1 : 0));
	...
	if ($sql->execute()) {
		...
		if ($_FILES['loc_uphoto']['tmp_name']) {
			...
			if (copy($_FILES['loc_uphoto']['tmp_name'], 'media/ourlocations/'.$id.'.'.$ext)) {
                ...
                <div class="box-body">
									<form role="form" method="post" enctype="multipart/form-data">
										...
										<div class="form-group">
											...
											<div class="controls">
												<input class="form-control" id="field_lat" name="field_lat" type="text" value="<? echo $row['loc_lat'] ?: $_POST['field_lat'];?>" placeholder="Latitude" />
												<input class="form-control" id="field_lon" name="field_lon" type="text" value="<? echo $row['loc_lng'] ?: $_POST['field_lon'];?>" placeholder="Longitude" />
											</div>
										</div>
										...
										<div class="row">
											...
											<div class="form-group col-sm-4">
												...
												<input type="text" class="form-control" name="loc_title_ar" required="required" value="<? echo $_POST['loc_title_ar'] ?: $row['loc_title_ar']?>" />
											</div>

											<div class="form-group col-sm-4">
												<label><?=LBL_TitleEn;?></label>
												<input type="text" class="form-control" name="loc_title_en" required="required" value="<? echo $_POST['loc_title_en'] ?: $row['loc_title_en']?>" />
											</div>

											<div class="form-group col-sm-4">
												<label><?=LBL_TitleUr;?></label>
												<input type="text" class="form-control" name="loc_title_ur" required="required" value="<? echo $_POST['loc_title_ur'] ?: $row['loc_title_ur']?>" />
											</div>
										...
										<div class="form-group">
											...
											<? if ($row['loc_photo']) echo '<img src="media/ourlocations/'.$row['loc_photo'].'" width="150"/>'; ?>
											<input id="loc_uphoto" name="loc_uphoto" type="file" class="file">
										</div>

										<div class="form-group">
											<label><?=LBL_ContactNumbers;?></label>
											<input type="text" class="form-control" name="loc_phones" value="<? echo $_POST['loc_phones'] ?: $row['loc_phones']?>" />
										</div>

										<div class="form-group">
											<label><?=LBL_Order;?></label>
											<input type="text" class="form-control" name="loc_order" value="<? echo $_POST['loc_order'] ?: $row['loc_order']?>" />
										</div>

										<div class="form-group">
											<label><input type="checkbox" name="loc_active" <? if (!$row || $row['loc_active'] == 1) echo 'checked="checked"';?> /> <?=LBL_Active;?></label>
										</div>
										...
										<input type="hidden" name="loc_dateadded" id="loc_dateadded" value="<?=$row['loc_dateadded'];?>" />
									</form>
                </div><!-- /.box-body -->
              </div><!-- /.box -->
            </div><!--/.col (left) -->
