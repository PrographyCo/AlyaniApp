	                ...
	                <?
										if ($_FILES) {
											...
											if ($_FILES['zipfile']['name']) {
												...
												if ($res === TRUE) {
												  $zip->extractTo('media/zip_photos/');
												  $zip->close();

													foreach (new DirectoryIterator('media/zip_photos') as $fileInfo) {
												    ...
												    if($fileInfo->isDot()) continue;
														...
														if (is_numeric($nationalid)) {
															...
															if(@is_array(getimagesize('media/zip_photos/'.$fileInfo->getFilename()))){
																...
																if ($pil_id > 0) {
																	...
																	// lets copy the image
																	$newname = GUID();
																	if (copy('media/zip_photos/'.$fileInfo->getFilename(), 'media/pils/'.$newname.'.'.$extension)) {
														...
														if (is_file('media/zip_photos/'.$fileInfo->getFilename())) @unlink('media/zip_photos/'.$fileInfo->getFilename());
														echo '<br />';
