...
if ($_POST) {
	...
	$sql->bindValue("promo_active", ($_POST['promo_active'] ? 1 : 0));
	...
	if ($sql->execute()) {
		...
		if ($_FILES['promo_uphoto']['tmp_name']) {
			...
			if (copy($_FILES['promo_uphoto']['tmp_name'], 'media/promos/'.$id.'.'.$ext)) {
									...
									<form role="form" method="post" enctype="multipart/form-data">
										...
										<div class="row">
											...
											<div class="form-group col-sm-4">
												...
												<input type="text" class="form-control" name="promo_title_ar" required="required" value="<? echo $_POST['promo_title_ar'] ?: $row['promo_title_ar']?>" />
											</div>

											<div class="form-group col-sm-4">
												<label><?=LBL_TitleEn;?></label>
												<input type="text" class="form-control" name="promo_title_en" required="required" value="<? echo $_POST['promo_title_en'] ?: $row['promo_title_en']?>" />
											</div>

											<div class="form-group col-sm-4">
												<label><?=LBL_TitleUr;?></label>
												<input type="text" class="form-control" name="promo_title_ur" required="required" value="<? echo $_POST['promo_title_ur'] ?: $row['promo_title_ur']?>" />
											</div>
										...
										<div class="row">
											...
											<div class="form-group col-sm-4">
												...
												<textarea name="promo_body_ar" class="form-control"><? echo $_POST['promo_body_ar'] ?: $row['promo_body_ar']?></textarea>
											</div>

											<div class="form-group col-sm-4">
												<label><?=LBL_DescrEn;?></label>
												<textarea name="promo_body_en" class="form-control"><? echo $_POST['promo_body_en'] ?: $row['promo_body_en']?></textarea>
											</div>

											<div class="form-group col-sm-4">
												<label><?=LBL_DescrUr;?></label>
												<textarea name="promo_body_ur" class="form-control"><? echo $_POST['promo_body_ur'] ?: $row['promo_body_ur']?></textarea>
											</div>
										...
										<div class="form-group">
											...
											<? if ($row['promo_photo']) echo '<img src="media/promos/'.$row['promo_photo'].'" width="150"/>'; ?>
										...
										<div class="form-group">
											...
											<input type="text" class="form-control" name="promo_order" value="<? echo $_POST['promo_order'] ?: $row['promo_order']?>" />
										</div>

										<div class="form-group">
											<label><input type="checkbox" name="promo_active" <? if (!$row || $row['promo_active'] == 1) echo 'checked="checked"';?> /> <?=LBL_Active;?></label>
										</div>

										<input type="submit" class="col-md-12 btn btn-success" value="<? echo $edit ? LBL_Update : LBL_Add; ?>" />
										<input type="hidden" name="id" id="id" value="<?=$_GET['id'];?>" />
										<input type="hidden" name="promo_photo" id="promo_photo" value="<?=$row['promo_photo'];?>" />
										<input type="hidden" name="promo_dateadded" id="promo_dateadded" value="<?=$row['promo_dateadded'];?>" />
									</form>
                </div><!-- /.box-body -->
              </div><!-- /.box -->
            </div><!--/.col (left) -->
