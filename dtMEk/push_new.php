              ...
              <div class="box box-info">
                <div class="box-body">
	                      ...
	                      <select name="sound" id="sound" class="form-control select2">
		                      <option value="default" <? if ($_POST['sound'] == 0) echo 'selected="selected"';?>><?=LBL_Default;?></option>
	                      ...
	                      <select name="platform" id="platform" class="form-control select2">
		                      <option value="0" <? if ($_POST['platform'] == 0) echo 'selected="selected"';?>><?=LBL_Both;?></option>
		                      <option value="1" <? if ($_POST['platform'] == 1) echo 'selected="selected"';?>><?=LBL_iPhone;?></option>
		                      <option value="2" <? if ($_POST['platform'] == 2) echo 'selected="selected"';?>><?=LBL_Android;?></option>
	                      </select>
	                  </div>

	                  <div class="form-group">
	                      <label><?=LBL_Type;?></label>
	                      <select name="type" id="type" class="form-control select2" onchange="checktype(this.value);">
		                      <option value="0" <? if ($_POST['type'] == 0) echo 'selected="selected"';?>><?=LBL_All;?></option>
													<option value="1" <? if ($_POST['type'] == 1) echo 'selected="selected"';?>><?=LBL_AllPilgrims;?></option>
													<option value="2" <? if ($_POST['type'] == 2) echo 'selected="selected"';?>><?=LBL_SPECIFICPIL;?></option>
		                      <option value="3" <? if ($_POST['type'] == 3) echo 'selected="selected"';?>><?=LBL_AllManagers;?></option>
													<option value="4" <? if ($_POST['type'] == 4) echo 'selected="selected"';?>><?=LBL_SPECIFICMANAGER;?></option>
													<option value="5" <? if ($_POST['type'] == 5) echo 'selected="selected"';?>><?=LBL_AllSupervisors;?></option>
													<option value="6" <? if ($_POST['type'] == 6) echo 'selected="selected"';?>><?=LBL_SPECIFICSUPERVISOR;?></option>
	                      </select>
	                  </div>
                  ...
                  <div class="form-group" id="pilarea" style="display:none">
	                  ...
	                  <select name="supervisor_staff_id" id="supervisor_staff_id" class="form-control select2">
											<option value="0">
												<?=LBL_Choose;?>
											</option>
		                  <?
