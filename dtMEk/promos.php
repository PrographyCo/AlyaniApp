<?php include 'layout/header.php';
    ...
    $table = 'promos';
    $table_id = 'promo_id';
    $newedit_page = 'e_promo.php';
    
    if (is_numeric($_GET['del'])) {
        
        $id = $_GET['del'];
        $photo = $db->query("SELECT promo_photo FROM $table WHERE $table_id = $id")->fetchColumn();
        if ($photo && $photo != 'default_photo.png' && is_file('media/promos/' . $photo)) @unlink('media/promos/' . $photo);
                        ...
                        <table class="datatable-table4 table table-bordered table-striped">
                            ...
                            <?
                                ...
                                while ($row = $sql->fetch()) {
                                    ...
                                    echo '<td>';
                                    echo '<img src="media/promos/' . $row['promo_photo'] . '" style="width:100px;" />';
                                    echo '</td>';
                                    echo '<td>';
                                    echo $row['promo_title_ar'];
                                    echo '</td>';
                                    echo '<td>';
                                    if ($row['promo_active'] == 1) echo '<span class="label label-success">' . LBL_Active . '</span>';
                                    else echo '<span class="label label-danger">' . LBL_Inactive . '</span>';
                                    echo '</td>';
                                    echo '<td>';
                                    echo $row['promo_order'];
                                    echo '</td>';
