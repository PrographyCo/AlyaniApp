                ...
                <div class="box-body">
									<form role="form" method="post" enctype="multipart/form-data">
										...
										<div class="form-group">
											...
											<input type="text" class="form-control" name="hall_title" required="required" value="<? echo $_POST['hall_title'] ?: $row['hall_title']?>" />
										</div>

										<div class="form-group">
											<label><?=LBL_Capacity;?></label>
											<input type="number" class="form-control" name="hall_capacity" value="<? echo $_POST['hall_capacity'] ?: $row['hall_capacity']?>" />
										</div>

										<div class="form-group">
											<label><?=LBL_Order;?></label>
											<input type="text" class="form-control" name="hall_order" value="<? echo $_POST['hall_order'] ?: $row['hall_order']?>" />
										</div>

										<div class="form-group">
											<label><input type="checkbox" name="hall_active" <? if (!$row || $row['hall_active'] == 1) echo 'checked="checked"';?> /> <?=LBL_Active;?></label>
										</div>

										<input type="submit" class="col-md-12 btn btn-success" value="<? echo $edit ? LBL_Update : LBL_Add; ?>" />
										<input type="hidden" name="id" id="id" value="<?=$_GET['id'];?>" />
										<input type="hidden" name="hall_dateadded" id="hall_dateadded" value="<?=$row['hall_dateadded'];?>" />
									</form>
                </div><!-- /.box-body -->
              </div><!-- /.box -->
            </div><!--/.col (left) -->
