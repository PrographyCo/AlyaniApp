                ...
                <div class="box-body">
									<form role="form" method="post" enctype="multipart/form-data">
										...
										<div class="form-group">
											...
											<input type="text" class="form-control" name="suite_title" required="required" value="<? echo $_POST['suite_title'] ?: $row['suite_title']?>" />
										</div>

										<div class="form-group">
											<label><?=LBL_Gender;?></label>
											<select name="suite_gender" class="form-control select2">
												<option value="m" <? if ($row['suite_gender'] == 'm') echo 'selected="selected"';?>><?=LBL_Male;?></option>
												<option value="f" <? if ($row['suite_gender'] == 'f') echo 'selected="selected"';?>><?=LBL_Female;?></option>
											</select>
										</div>

										<div class="form-group">
											<label><?=LBL_Order;?></label>
											<input type="text" class="form-control" name="suite_order" value="<? echo $_POST['suite_order'] ?: $row['suite_order']?>" />
										</div>

										<div class="form-group">
											<label><input type="checkbox" name="suite_active" <? if (!$row || $row['suite_active'] == 1) echo 'checked="checked"';?> /> <?=LBL_Active;?></label>
										</div>

										<input type="submit" class="col-md-12 btn btn-success" value="<? echo $edit ? LBL_Update : LBL_Add; ?>" />
										<input type="hidden" name="id" id="id" value="<?=$_GET['id'];?>" />
										<input type="hidden" name="suite_dateadded" id="suite_dateadded" value="<?=$row['suite_dateadded'];?>" />
									</form>
                </div><!-- /.box-body -->
              </div><!-- /.box -->
            </div><!--/.col (left) -->
