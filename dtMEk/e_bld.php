                ...
                <div class="box-body">
									<form role="form" method="post" enctype="multipart/form-data">
										...
										<div class="form-group">
											...
											<input type="text" class="form-control" name="bld_title" required="required" value="<? echo $_POST['bld_title'] ?: $row['bld_title']?>" />
										</div>

										<div class="form-group">
											<label><?=LBL_Type;?></label>
											<select name="bld_type" class="form-control select2">
												<option value="1" <? if ($row['bld_type'] == 1) echo 'selected="selected"'; ?>>
													<?=HM_Building;?>
												</option>
												<option value="2" <? if ($row['bld_type'] == 2) echo 'selected="selected"'; ?>>
													<?=LBL_Premises;?>
												</option>
											</select>
										</div>
										...
										<div class="form-group">
											...
											<input type="text" class="form-control" name="bld_order" value="<? echo $_POST['bld_order'] ?: $row['bld_order']?>" />
										</div>

										<div class="form-group">
											<label><input type="checkbox" name="bld_active" <? if (!$row || $row['bld_active'] == 1) echo 'checked="checked"';?> /> <?=LBL_Active;?></label>
										</div>

										<input type="submit" class="col-md-12 btn btn-success" value="<? echo $edit ? LBL_Update : LBL_Add; ?>" />
										<input type="hidden" name="id" id="id" value="<?=$_GET['id'];?>" />
										<input type="hidden" name="bld_dateadded" id="bld_dateadded" value="<?=$row['bld_dateadded'];?>" />
									</form>
                </div><!-- /.box-body -->
              </div><!-- /.box -->
            </div><!--/.col (left) -->
