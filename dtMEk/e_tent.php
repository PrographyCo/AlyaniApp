                ...
                <div class="box-body">
									<form role="form" method="post" enctype="multipart/form-data">
										...
										<div class="form-group">
											<label><?php if(isset($_GET['type']) && $_GET['type'] == 2){echo LBL_TentNumber_halls;}else{echo LBL_TentNumber;};?></label>
											<input type="text" class="form-control" name="tent_title" required="required" value="<? echo $_POST['tent_title'] ?: $row['tent_title']?>" />
										</div>

										<div class="form-group">
											<label><?=LBL_Type;?></label>
											<select name="tent_type" class="form-control select2">
												<option value="1" <? if ($row['tent_type'] == 1) echo 'selected="selected"';?>><?=LBL_TentType1;?></option>
												<option value="2" <? if ($row['tent_type'] == 2) echo 'selected="selected"';?>><?=LBL_TentType2;?></option>
											</select>
										</div>
										...
										<div class="form-group">
											...
											<select name="tent_gender" class="form-control select2">
												<option value="m" <? if ($row['tent_gender'] == 'm') echo 'selected="selected"';?>><?=LBL_Male;?></option>
												<option value="f" <? if ($row['tent_gender'] == 'f') echo 'selected="selected"';?>><?=LBL_Female;?></option>
											</select>
										</div>

										<div class="form-group">
											<label><?=LBL_Capacity;?></label>
											<input type="number" class="form-control" name="tent_capacity" value="<? echo $_POST['tent_capacity'] ?: $row['tent_capacity']?>" />
										</div>

										<div class="form-group">
											<label><?=LBL_Order;?></label>
											<input type="text" class="form-control" name="tent_order" value="<? echo $_POST['tent_order'] ?: $row['tent_order']?>" />
										</div>

										<div class="form-group">
											<label><input type="checkbox" name="tent_active" <? if (!$row || $row['tent_active'] == 1) echo 'checked="checked"';?> /> <?=LBL_Active;?></label>
										</div>

										<input type="submit" class="col-md-12 btn btn-success" value="<? echo $edit ? LBL_Update : LBL_Add; ?>" />
										<input type="hidden" name="id" id="id" value="<?=$_GET['id'];?>" />
										<input type="hidden" name="tent_dateadded" id="tent_dateadded" value="<?=$row['tent_dateadded'];?>" />
									</form>
                </div><!-- /.box-body -->
              </div><!-- /.box -->
            </div><!--/.col (left) -->
