...
<!-- Content Wrapper. Contains page content -->
   ...
   <style type="text/css">
      ...
      <div class="content-wrapper">
       ...
       <button  class="btn btn-success pull-<?=DIR_AFTER;?>" onclick="pdfsuites()"  style="margin-<?=DIR_AFTER;?>: 10px" target="_blank"><i class="fa fa-file-pdf-o"></i> <?=BTN_ExportToPDF;?></button>
       <a href="?remove=1&suite_id=<?=$_GET['suite_id'];?>&hall_id=<?=$_GET['hall_id'];?>" onclick="return confirm('<?=LBL_RemoveConfirm;?>');" class="btn btn-danger pull-<?=DIR_AFTER;?>" style="margin-<?=DIR_AFTER;?>: 10px"><i class="fa fa-trash"></i> <?=BTN_REMOVEACCOMO2;?></a>
          </h1>
        </section>
        ...
        <section class="content">
          <div class="row">
            ...
            <div class="col-md-12">
										...
										<div class="row">
												...
												<select name="suite_id" id="suite_id" class="form-control select2">
													...
													<?
														...
														while ($rows = $sqlsuites->fetch(PDO::FETCH_ASSOC)){
															...
															if ($_GET['suite_id'] == $rows['suite_id']) echo 'selected="selected"';
															echo '>
															'.$rows['suite_title'].'
															</option>';
														}
											...
											<div class="form-group col-sm-6">
												...
												<select name="hall_id" id="hall_id" class="form-control select2">
													...
													<?
													if (is_numeric($_GET['suite_id']) && $_GET['suite_id'] > 0) {
														...
														while ($rowh = $sqlhalls->fetch(PDO::FETCH_ASSOC)){
															...
															if ($_GET['hall_id'] == $rowh['hall_id']) echo 'selected="selected"';
															echo '>
															'.$rowh['hall_title'].'
															</option>';
														}
							        ...
							        <?php if($_GET['hall_id']){
                                        ...
                                        ?>
                                            ...
                                            <div style="padding: 26px;font-size: 18px;">
                                                             <p><?php echo LBL_SuiteNumber.': '.$suite['suite_title'] ." - ".HM_Hall.': '.$hall['hall_title'];?></p>
                ...
                <div class="box-body">
                    <?php 	if (is_numeric($_GET['suite_id']) && $_GET['hall_id'] > 0){ ?>
                               <table class=" table table-bordered table-striped" id="myTable2" cellspacing="0" cellpadding="0">
                    <thead>
                      <tr>
                            <th><?=LBL_Name;?></th>
