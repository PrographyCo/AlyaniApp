...
if ($_POST) {
	...
	if ($chk->rowCount() > 0) {
		...
		try {
			...
			$sql->bindValue("bus_active", ($_POST['bus_active'] ? 1 : 0));
                ...
                <div class="box-body">
									<form role="form" method="post" enctype="multipart/form-data">
										...
										<div class="form-group">
											...
											<input type="text" class="form-control" name="bus_title" required="required" value="<? echo $_POST['bus_title'] ?: $row['bus_title']?>" />
										</div>
										...
										<div class="form-group">
											...
											<input type="number" class="form-control" name="bus_seats" value="<? echo $_POST['bus_seats'] ?: $row['bus_seats']?>" />
										</div>
										...
										<div class="form-group">
											...
											<select name="bus_staff_id" class="form-control select2">
												...
												while ($row2 = $sql2->fetch(PDO::FETCH_ASSOC)) {
													...
													if ($row['bus_staff_id'] == $row2['staff_id']) echo 'selected="selected"';
										...
										<div class="form-group">
											...
											<input type="text" class="form-control" name="bus_order" value="<? echo $_POST['bus_order'] ?: $row['bus_order']?>" />
										</div>

										<div class="form-group">
											<label><input type="checkbox" name="bus_active" <? if (!$row || $row['bus_active'] == 1) echo 'checked="checked"';?> /> <?=LBL_Active;?></label>
										</div>

										<input type="submit" class="col-md-12 btn btn-success" value="<? echo $edit ? LBL_Update : LBL_Add; ?>" />
										<input type="hidden" name="id" id="id" value="<?=$_GET['id'];?>" />
										<input type="hidden" name="bus_dateadded" id="bus_dateadded" value="<?=$row['bus_dateadded'];?>" />
									</form>
                </div><!-- /.box-body -->
              </div><!-- /.box -->
            </div><!--/.col (left) -->
