<?
  ...
  $selected = $_POST['selected'] ?: $_GET['selected'];

  $output['html'] = '';
  if (is_array($suites) && sizeof($suites)) {
    ...
    while ($row1 = $sqlhalls->fetch(PDO::FETCH_ASSOC)) {
      ...
      if (is_array($selected) && sizeof($selected) > 0 && in_array($row1['hall_id'], $selected)) $output['html'] .= 'selected="selected"';
      $output['html'] .= '>
      '.$row1['hall_title'].'
      </option>';
