                    ...
                    <div class="box-body">
                        <form method="post" enctype="multipart/form-data">
                            ...
                            <div class="panel well">
                                <?= ($type==='emp')?HM_STAFF:LBL_Pilgrim ?>: <span id="countpils"> <?= $pil_code ?> </span>
                            ...
                            <div class="row">
                                <div class="form-group col-sm-4">
                                    ...
                                    <select name="gender" id="gender" class="form-control select2"
                                            onchange=" calcAvailAccomo(); getAccomosGender(this.value);">
                                        <option value="m" <?php if (($_REQUEST['gender']??$gender) === 'm') echo 'selected="selected"'; ?>>
                                            <?= LBL_Male ?>
                                        </option>
                                        <option value="f" <?php if (($_REQUEST['gender']??$gender) === 'f') echo 'selected="selected"'; ?>>
                                            <?= LBL_Female ?>
                                        </option>
                                    </select>
                                </div>
                            ...
                            <input type="submit" class="btn btn-primary col-xs-12" value="<?= LBL_ApplyAccomo ?>" disabled/>
...
<script>
    ...
    $(document).ready(function () {
        $('select').select2();
        suite.val(["<?= $accomo?$accomo['suite_id']:'' ?>"]).trigger('change');
        
        <?php
        if ($accomo) {
            $bld_type = $db->query('SELECT bld_type FROM buildings WHERE bld_id = '.$accomo['bld_id'])->fetchColumn()?:'';
        ...
        $('#tent_id\\[\\]').val(["<?= $accomo?$accomo['tent_id']:'' ?>"]).trigger('change');
    });
