<?php
	...
	:news_lastupdated
            ...
            $sql->bindValue("news_active", (isset($_POST['news_active']) ? 1 : 0));
                        ...
                        <form role="form" method="post" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-sm-4">
                                    ...
                                    <textarea name="news_body_ar"
                                              class="form-control"><?= $_POST['news_body_ar'] ?? $row['news_body_ar'] ?? '' ?></textarea>
                                </div>

                                <div class="form-group col-sm-4">
                                    <label><?= LBL_DescrEn ?></label>
                                    <textarea name="news_body_en"
                                              class="form-control"><?= $_POST['news_body_en'] ?? $row['news_body_en'] ?? '' ?></textarea>
                                </div>

                                <div class="form-group col-sm-4">
                                    <label><?= LBL_DescrUr ?></label>
                                    <textarea name="news_body_ur"
                                              class="form-control"><?= $_POST['news_body_ur'] ?? $row['news_body_ur'] ?? '' ?></textarea>
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <?php if (isset($row['news_photo']) && $row['news_photo']) echo '<img src="'.CP_PATH.'/assets/media/news/' . $row['news_photo'] . '" width="150"/>'; ?>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="news_order"
                                       value="<?= $_POST['news_order'] ?? $row['news_order'] ?? '' ?>"/>
                            </div>

                            <div class="form-group">
                                <label><input type="checkbox"
                                              name="news_active" <?php if (!isset($row) || $row['news_active'] == 1) echo 'checked="checked"'; ?> /> <?= LBL_Active ?>
                                </label>
                            </div>
                            ...
                            <input type="hidden" name="news_dateadded" id="news_dateadded"
                                   value="<?= $row['news_dateadded']??time() ?>"/>
                        </form>
                    </div><!-- /.box-body -->
                </div><!-- /.box -->
            </div><!--/.col (left) -->
