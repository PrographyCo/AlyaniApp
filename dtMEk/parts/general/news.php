<?php
    ...
    $table = 'news';
    $table_id = 'news_id';
    $newedit_page = CP_PATH.'/general/edit/new';
    
    if (is_numeric($_GET['del']??'')) {
        
        $id = $_GET['del'];
        $photo = $db->query("SELECT news_photo FROM $table WHERE $table_id = $id")->fetchColumn();
        if ($photo && $photo !== 'default_photo.png' && is_file(ASSETS_PATH.'media/news/' . $photo)) @unlink(ASSETS_PATH.'media/news/' . $photo);
                ...
                <div class="box">
                    <div class="box-body">
                        <table class="datatable-table4 table table-bordered table-striped">
                            ...
                            <tbody>
                            <?php
                                $sql = $db->query("SELECT * FROM $table ORDER BY news_order");
                                while ($row = $sql->fetch()) {
                                    ...
                                    echo '</td>';
                                    echo '<td>';
                                    if ($row['news_active'] == 1) echo '<span class="label label-success">' . LBL_Active . '</span>';
