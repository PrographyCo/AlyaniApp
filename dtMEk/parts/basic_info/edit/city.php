<?php
    ...
    if ($_POST) {
        ...
        try {
            ...
            $sql->bindValue("city_active", (isset($_POST['city_active']) ? 1 : 0));
            ...
            if ($sql->execute()) {
                ...
                if (isset($_POST['city_sv']) && is_array($_POST['city_sv']) && count($_POST['city_sv']) > 0) {
                    ...
                    <div class="box-body">
                        <form role="form" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="city_prefix" required="required"
                                       value="<?= $_POST['city_prefix'] ?? $row['city_prefix'] ?? '' ?>"/>
                            </div>

                            <div class="form-group">
                                <label><?= LBL_Order ?></label>
                                <input type="text" class="form-control" name="city_order"
                                       value="<?= $_POST['city_order'] ?? $row['city_order'] ?? '' ?>"/>
                            </div>
                                ...
                                <select name="city_sv[]" class="form-control select2" multiple="multiple">
                                    <?php
                                        ...
                                        while ($row2 = $sql2->fetch(PDO::FETCH_ASSOC)) {
                                            ...
                                            if (in_array($row2['staff_id'], $sv_array, true) || (is_array($_POST['city_sv']) && in_array($row2['staff_id'], $_POST['city_sv'], true))) echo 'selected="selected"';
