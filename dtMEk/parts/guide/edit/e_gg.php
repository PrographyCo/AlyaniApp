<?php
	...
	:gg_lastupdated
            ...
            $sql->bindValue("gg_active", (isset($_POST['gg_active']) ? 1 : 0));
            ...
            if ($sql->execute()) {
                ...
                if ($_FILES['gg_uphoto']['tmp_name']) {
                    ...
                    if (copy($_FILES['gg_uphoto']['tmp_name'], ASSETS_PATH.'media/gg/' . $id . '.' . $ext)) {
                ...
                if (isset($_POST['gg_sv']) && is_array($_POST['gg_sv']) && count($_POST['gg_sv']) > 0) {
                        ...
                        <form role="form" method="post" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-sm-4">
                                    ...
                                    <textarea name="gg_descr_ar"
                                              class="form-control"><?= $_POST['gg_descr_ar'] ?? $row['gg_descr_ar'] ?? '' ?></textarea>
                                </div>

                                <div class="form-group col-sm-4">
                                    <label><?= LBL_DescrEn ?></label>
                                    <textarea name="gg_descr_en"
                                              class="form-control"><?= $_POST['gg_descr_en'] ?? $row['gg_descr_en'] ?? '' ?></textarea>
                                </div>

                                <div class="form-group col-sm-4">
                                    <label><?= LBL_DescrUr ?></label>
                                    <textarea name="gg_descr_ur"
                                              class="form-control"><?= $_POST['gg_descr_ur'] ?? $row['gg_descr_ur'] ?? '' ?></textarea>
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="gg_order"
                                       value="<?= $_POST['gg_order'] ?? $row['gg_order'] ?? '' ?>"/>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="gg_sv[]" class="form-control select2" multiple="multiple">
                                    <?php
                                        if (isset($row)) $sv_array = $db->query("SELECT ggs_id FROM gg_staff WHERE gg_id = " . $row['gg_id'] . " AND ggs_id IN (SELECT ggs_id FROM general_guide_staff WHERE ggs_active = 1)")->fetchAll(PDO::FETCH_COLUMN);
                                        else $sv_array = array();
                            ...
                            <div class="form-group">
                                ...
                                <?php if (isset($row) && $row['gg_photo']) echo '<img src="'.CP_PATH.'/assets/media/gg/' . $row['gg_photo'] . '" width="150"/>'; ?>
                            ...
                            <div class="form-group">
                                <label><input type="checkbox"
                                              name="gg_active" <?php if (!isset($row) || $row['gg_active'] == 1) echo 'checked="checked"'; ?> /> <?= LBL_Active ?>
                                </label>
                            </div>
