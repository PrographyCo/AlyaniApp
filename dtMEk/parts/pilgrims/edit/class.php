<?php
	...
	:pilc_lastupdated
            ...
            $sql->bindValue("pilc_active", (isset($_POST['pilc_active']) ? 1 : 0));
            ...
            if ($sql->execute()) {
                ...
                // Handling Muftis
                if (isset($_POST['pilc_muftis']) && is_array($_POST['pilc_muftis']) && count($_POST['pilc_muftis']) > 0) {
                ...
                if (isset($_POST['pilc_sv']) && is_array($_POST['pilc_sv']) && count($_POST['pilc_sv']) > 0) {
                        ...
                        <form role="form" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="pilc_order"
                                       value="<?= $_POST['pilc_order'] ?? $row['pilc_order'] ?? '' ?>"/>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="pilc_muftis[]" class="form-control select2" multiple="multiple">
                                    <?php
                                        if (isset($row)) $muftis_array = $db->query("SELECT staff_id FROM pils_classes_muftis WHERE pilc_id = " . $row['pilc_id'] . " AND staff_id IN (SELECT staff_id FROM staff WHERE staff_type = 3 AND staff_active = 1)")->fetchAll(PDO::FETCH_COLUMN);
                                ...
                                <select name="pilc_city1_staff_id" class="form-control select2">
                                    ...
                                    <?php
                                        ...
                                        while ($row1 = $sql1->fetch(PDO::FETCH_ASSOC)) {
                                            ...
                                            echo '<option value="' . $row1['staff_id'] . '" ';
                                            if (isset($row) && $row['pilc_city1_staff_id'] == $row1['staff_id']) echo 'selected="selected"';
                                            echo '>' . $row1['staff_name'] . '</option>';
                                ...
                                <select name="pilc_city2_staff_id" class="form-control select2">
                                    ...
                                    <?php
                                        ...
                                        while ($row1 = $sql1->fetch(PDO::FETCH_ASSOC)) {
                                            ...
                                            echo '<option value="' . $row1['staff_id'] . '" ';
                                            if (isset($row) && $row['pilc_city2_staff_id'] == $row1['staff_id']) echo 'selected="selected"';
                                            echo '>' . $row1['staff_name'] . '</option>';
                                ...
                                <select name="pilc_city3_staff_id" class="form-control select2">
                                    ...
                                    <?php
                                        ...
                                        while ($row1 = $sql1->fetch(PDO::FETCH_ASSOC)) {
                                            ...
                                            echo '<option value="' . $row1['staff_id'] . '" ';
                                            if (isset($row) && $row['pilc_city3_staff_id'] == $row1['staff_id']) echo 'selected="selected"';
                                            echo '>' . $row1['staff_name'] . '</option>';
                            ...
                            <div class="row">
                                <div class="form-group col-sm-6">
                                    ...
                                    <select name="pilc_text_id" id="pilc_text_id" class="form-control select2">
                                        <option value="1" <?php if (isset($row) && $row['pilc_text_id'] == 1) echo 'selected="selected"'; ?>><?= LBL_Mokhayam ?></option>
                                        <option value="2" <?php if (isset($row) && $row['pilc_text_id'] == 2) echo 'selected="selected"'; ?>><?= LBL_Omara ?></option>
                                    </select>
                                </div>
                                ...
                                <div class="form-group col-sm-6">
                                    ...
                                    <input type="text" class="form-control" name="pilc_text_text" required="required"
                                           value="<?= $_POST['pilc_text_text'] ?? $row['pilc_text_text'] ?? '' ?>"/>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="pilc_phones"
                                       value="<?= $_POST['pilc_phones'] ?? $row['pilc_phones'] ?? '' ?>"/>
                            </div>
                                ...
                                <div class="row">
                                    <?php
                                        for ($i = 1; $i <= 10; $i++) {
                                            ...
                                            if (isset($row) && $row['pilc_design_id'] == $i) echo 'checked="checked"';
                                            echo '/> <img src="'.CP_PATH.'/assets/media/cards/pils/common/' . $i . '.png" style="height:300px;" /></label>
														</div>';
                            ...
                            <div class="form-group">
                                <label><input type="checkbox"
                                              name="pilc_active" <?php if (!isset($row) || $row['pilc_active'] == 1) echo 'checked="checked"'; ?> /> <?= LBL_Active ?>
                                </label>
                            </div>
