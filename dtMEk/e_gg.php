...
if ($_POST) {
	...
	if ($sql->execute()) {
		...
		if (is_array($_POST['gg_sv']) && sizeof($_POST['gg_sv']) > 0) {
									...
									<form role="form" method="post" enctype="multipart/form-data">
										...
										<div class="row">
											...
											<div class="form-group col-sm-4">
												...
												<input type="text" class="form-control" name="gg_title_ar" required="required" value="<? echo $_POST['gg_title_ar'] ?: $row['gg_title_ar']?>" />
											</div>

											<div class="form-group col-sm-4">
												<label><?=LBL_TitleEn;?></label>
												<input type="text" class="form-control" name="gg_title_en" required="required" value="<? echo $_POST['gg_title_en'] ?: $row['gg_title_en']?>" />
											</div>

											<div class="form-group col-sm-4">
												<label><?=LBL_TitleUr;?></label>
												<input type="text" class="form-control" name="gg_title_ur" required="required" value="<? echo $_POST['gg_title_ur'] ?: $row['gg_title_ur']?>" />
											</div>
										...
										<div class="row">
											...
											<div class="form-group col-sm-4">
												...
												<textarea name="gg_descr_ar" class="form-control"><? echo $_POST['gg_descr_ar'] ?: $row['gg_descr_ar']?></textarea>
											</div>

											<div class="form-group col-sm-4">
												<label><?=LBL_DescrEn;?></label>
												<textarea name="gg_descr_en" class="form-control"><? echo $_POST['gg_descr_en'] ?: $row['gg_descr_en']?></textarea>
											</div>

											<div class="form-group col-sm-4">
												<label><?=LBL_DescrUr;?></label>
												<textarea name="gg_descr_ur" class="form-control"><? echo $_POST['gg_descr_ur'] ?: $row['gg_descr_ur']?></textarea>
											</div>
										...
										<div class="form-group">
											...
											<input type="text" class="form-control" name="gg_order" value="<? echo $_POST['gg_order'] ?: $row['gg_order']?>" />
										</div>
										...
										<div class="form-group">
											...
											<? if ($row['gg_photo']) echo '<img src="media/gg/'.$row['gg_photo'].'" width="150"/>'; ?>
											<input id="gg_uphoto" name="gg_uphoto" type="file" class="file">
										</div>


										<div class="form-group">
											<label><input type="checkbox" name="gg_active" <? if (!$row || $row['gg_active'] == 1) echo 'checked="checked"';?> /> <?=LBL_Active;?></label>
										</div>
										...
										<input type="hidden" name="gg_dateadded" id="gg_dateadded" value="<?=$row['gg_dateadded'];?>" />
									</form>
                </div><!-- /.box-body -->
              </div><!-- /.box -->
            </div><!--/.col (left) -->
