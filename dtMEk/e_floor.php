...
if ($_POST) {
	...
	if ($chk->rowCount() > 0) {
		...
		try {
			...
			$sql->bindValue("floor_active", ($_POST['floor_active'] ? 1 : 0));
                ...
                <div class="box-body">
									<form role="form" method="post" enctype="multipart/form-data">
										...
										<div class="form-group">
											...
											<select name="floor_bld_id" class="form-control select2">
												...
												while ($rowb = $sqlb->fetch(PDO::FETCH_ASSOC)) {
													...
													if ($row['floor_bld_id'] == $rowb['bld_id']) echo 'selected="selected"';
										...
										<div class="form-group">
											...
											<input type="text" class="form-control" name="floor_title" required="required" value="<? echo $_POST['floor_title'] ?: $row['floor_title']?>" />
										</div>

										<div class="form-group">
											<label><?=LBL_Order;?></label>
											<input type="text" class="form-control" name="floor_order" value="<? echo $_POST['floor_order'] ?: $row['floor_order']?>" />
										</div>

										<div class="form-group">
											<label><input type="checkbox" name="floor_active" <? if (!$row || $row['floor_active'] == 1) echo 'checked="checked"';?> /> <?=LBL_Active;?></label>
										</div>

										<input type="submit" class="col-md-12 btn btn-success" value="<? echo $edit ? LBL_Update : LBL_Add; ?>" />
										<input type="hidden" name="id" id="id" value="<?=$_GET['id'];?>" />
										<input type="hidden" name="floor_dateadded" id="floor_dateadded" value="<?=$row['floor_dateadded'];?>" />
									</form>
                </div><!-- /.box-body -->
              </div><!-- /.box -->
            </div><!--/.col (left) -->
