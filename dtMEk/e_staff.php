                ...
                <div class="box-body">
									<form role="form" method="post" enctype="multipart/form-data">
										...
										<div class="form-group">
											...
											<select name="staff_type" class="form-control select2" onchange="typechanged(this.value);">
												<option value="1" <? if ($row['staff_type'] == 1 || $_GET['type'] == 1) echo 'selected="selected"';?>><?=HM_Managers;?></option>
												<option value="2" <? if ($row['staff_type'] == 2 || $_GET['type'] == 2) echo 'selected="selected"';?>><?=HM_Supervisors;?></option>
												<option value="3" <? if ($row['staff_type'] == 3 || $_GET['type'] == 3) echo 'selected="selected"';?>><?=HM_Muftis;?></option>
											</select>
										</div>

										<div class="form-group">
											<label><?=LBL_Name;?></label>
											<input type="text" class="form-control" name="staff_name" required="required" value="<? echo $_POST['staff_name'] ?: $row['staff_name']?>" />
										</div>

										<div class="form-group">
											<label><?=LBL_Email;?></label>
											<input type="email" class="form-control" name="staff_email" value="<? echo $_POST['staff_email'] ?: $row['staff_email']?>" />
										</div>

										<div class="form-group">
											<label><?=LBL_ContactNumbers;?></label>
											<input type="text" class="form-control" name="staff_phones" value="<? echo $_POST['staff_phones'] ?: $row['staff_phones']?>" />
										</div>

										<div class="form-group">
											<label><?=LBL_Photo;?></label><br />
											<? if ($row['staff_photo']) echo '<img src="media/staff/'.$row['staff_photo'].'" width="150"/>'; ?>
											<input id="staff_uphoto" name="staff_uphoto" type="file" class="file">
										</div>

										<div id="privaccess" <? if ($row['staff_type'] == 1 || $_GET['type'] == 1 || $row['staff_type'] == 3 || $_GET['type'] == 3) echo 'style="display: none"';?>>
											<div class="form-group">
												<label><?=LBL_Permissions;?></label>
												<select name="staff_priv" class="form-control select2">
													<option value="1" <? if ($row['staff_priv'] == 1 || $_GET['type'] == 1) echo 'selected="selected"';?>><?=LBL_Locations;?></option>
													<option value="2" <? if ($row['staff_priv'] == 2 || $_GET['type'] == 2) echo 'selected="selected"';?>><?=LBL_Buses;?></option>
												</select>
											</div>
										</div>

										<div id="appaccess" <? if ($row['staff_type'] == 3 || $_GET['type'] == 3) echo 'style="display: none"';?>>
											<div class="form-group">
												<label><?=LBL_Username;?></label>
												<input type="text" class="form-control" name="staff_username" value="<? echo $_POST['staff_username'] ?: $row['staff_username']?>" />
											</div>

											<div class="form-group">
												<label><? echo !$row ? LBL_Password : LBL_ApplyNewPassword;?> - <a href="#" onclick="generateRand(); return false;"><?=LBL_GenerateRandom;?></a> - <a href="#" onclick="togglepasstype(); return false;"><?=LBL_ViewHide;?></a></label>
												<input type="password" class="form-control" name="newpassword" id="newpassword" value="" autocomplete="off" />
											</div>
										</div>

										<div class="form-group">
											<label><input type="checkbox" name="staff_active" <? if (!$row || $row['staff_active'] == 1) echo 'checked="checked"';?> /> <?=LBL_Active;?></label>
										</div>

										<input type="submit" class="col-md-12 btn btn-success" value="<? echo $edit ? LBL_Update : LBL_Add; ?>" />
										<input type="hidden" name="id" id="id" value="<?=$_GET['id'];?>" />
										<input type="hidden" name="oldpassword" id="oldpassword" value="<?=$row['staff_password'];?>" />
										<input type="hidden" name="staff_photo" id="staff_photo" value="<?=$row['staff_photo'];?>" />
										<input type="hidden" name="staff_dateadded" id="staff_dateadded" value="<?=$row['staff_dateadded'];?>" />
									</form>
                </div><!-- /.box-body -->
              </div><!-- /.box -->
            </div><!--/.col (left) -->
