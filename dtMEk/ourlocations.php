<?php include 'header.php';
    ...
    $table = 'ourlocations';
    $table_id = 'loc_id';
    $newedit_page = 'e_loc.php';
    
    if (is_numeric($_GET['del'])) {
        
        $id = $_GET['del'];
        $photo = $db->query("SELECT loc_photo FROM $table WHERE $table_id = $id")->fetchColumn();
        if ($photo && $photo != 'default_photo.png' && is_file('media/ourlocations/' . $photo)) @unlink('media/ourlocations/' . $photo);
                ...
                <div class="box">
                    <div class="box-body">
                        <table class="datatable-table4 table table-bordered table-striped">
                            ...
                            <tbody>
                            <?
                                $sql = $db->query("SELECT * FROM $table ORDER BY loc_order");
                                while ($row = $sql->fetch()) {
                                    
                                    echo '<tr>';
                                    echo '<td>';
                                    echo $row['loc_title_ar'];
                                    echo '</td>';
                                    echo '<td>';
                                    echo $row['loc_phones'];
                                    echo '</td>';
                                    echo '<td>';
                                    if ($row['loc_active'] == 1) echo '<span class="label label-success">' . LBL_Active . '</span>';
